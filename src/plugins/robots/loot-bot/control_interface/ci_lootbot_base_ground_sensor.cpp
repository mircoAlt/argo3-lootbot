...
/**
 * @file <argos3/plugins/robots/loot-bot/control_interface/ci_lootbot_base_ground_sensor.cpp>
 *
 * @brief This file provides the implementation of the lootbot base ground sensor.
 *
...
#include "ci_lootbot_base_ground_sensor.h"
...
namespace argos {
   ...
   CCI_LootBotBaseGroundSensor::CCI_LootBotBaseGroundSensor() :
      m_tReadings(8) {
   ...
   const CCI_LootBotBaseGroundSensor::TReadings& CCI_LootBotBaseGroundSensor::GetReadings() const {
     return m_tReadings;
   }
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LootBotBaseGroundSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LootBotBaseGroundSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LootBotBaseGroundSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LootBotBaseGroundSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
