/**
 * @file <argos3/plugins/robots/loot-bot/control_interface/ci_lootbot_light_sensor.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "ci_lootbot_light_sensor.h"
...
namespace argos {
   ...
   CCI_LootBotLightSensor::CCI_LootBotLightSensor() :
      m_tReadings(24) {
   ...
   const CCI_LootBotLightSensor::TReadings& CCI_LootBotLightSensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_LootBotLightSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_LootBotLightSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LootBotLightSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_LootBotLightSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
