/**
 * @file <argos3/plugins/robots/loot-bot/control_interface/ci_lootbot_turret_actuator.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "ci_lootbot_turret_actuator.h"
...
namespace argos {
  ...
  const CRange<SInt32> CCI_LootBotTurretActuator::SPEED_RANGE(-4,4);
  const CRange<Real>   CCI_LootBotTurretActuator::NORMALIZED_SPEED_RANGE(-1.0,1.0);
  ...
  int LuaTurretSetRotation(lua_State* pt_lua_state) {
    ...
    /* Perform action */
    CLuaUtility::GetDeviceInstance<CCI_LootBotTurretActuator>(pt_lua_state, "turret")->
  ...
  void CCI_LootBotTurretActuator::SetActiveWithRotation(const CRadians& c_angle) {
    SetPositionControlMode();
    SetRotation(c_angle);
  }
  ...
  void CCI_LootBotTurretActuator::SetSpeedControlMode() {
  ...
  void CCI_LootBotTurretActuator::SetPositionControlMode() {
  ...
  void CCI_LootBotTurretActuator::SetPassiveMode() {
...
#ifdef ARGOS_WITH_LUA
  void CCI_LootBotTurretActuator::CreateLuaState(lua_State* pt_lua_state) {
