/**
 * @file "../simulator/lootbot_motor_ground_rotzonly_sensor.cpp"
 *
...
#include "lootbot_motor_ground_rotzonly_sensor.h"
...
namespace argos {
   ...
   CLootBotMotorGroundRotZOnlySensor::CLootBotMotorGroundRotZOnlySensor() :
   ...
   void CLootBotMotorGroundRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
   ...
   void CLootBotMotorGroundRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_LootBotMotorGroundSensor::Init(t_tree);
         /* Parse noise level */
         Real fNoiseLevel = 0.0f;
         GetNodeAttributeOrDefault(t_tree, "noise_level", fNoiseLevel, fNoiseLevel);
         if(fNoiseLevel < 0.0f) {
            THROW_ARGOSEXCEPTION("Can't specify a negative value for the noise level of the loot-bot ground sensor");
         }
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in loot-bot rotzonly ground sensor", ex);
      }
   }
   ...
   void CLootBotMotorGroundRotZOnlySensor::Update() {
      /*
   ...
   void CLootBotMotorGroundRotZOnlySensor::Reset() {
   ...
   REGISTER_SENSOR(CLootBotMotorGroundRotZOnlySensor,
                   "lootbot_motor_ground", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The loot-bot motor ground sensor.",
                   "This sensor accesses the loot-bot motor ground sensor. For a complete description\n"
                   "of its usage, refer to the ci_lootbot_motor_ground_sensor.h interface. For the XML\n"
