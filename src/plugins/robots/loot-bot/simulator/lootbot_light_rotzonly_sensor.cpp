/**
 * @file "../simulator/lootbot_light_rotzonly_sensor.cpp"
 *
...
#include "lootbot_light_rotzonly_sensor.h"
...
namespace argos {
   ...
   CLootBotLightRotZOnlySensor::CLootBotLightRotZOnlySensor() :
      m_bEnabled(true),
   ...
   void CLootBotLightRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the loot-bot light default sensor", ex);
      }
   }
   ...
   void CLootBotLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CLootBotLightRotZOnlySensor::Update() {
      if (!m_bEnabled) {
        return;
      }
      /* Erase readings */
      ...
      /* Buffer for the angle of the light wrt to the loot-bot */
      CRadians cAngleLightWrtLootbot;
       ...
       */
         for(CSpace::TMapPerType::iterator it = mapLights.begin();
            ...
            if(cLight.GetIntensity() > 0.0f) {
               ...
               /* Check occlusion between the loot-bot and the light */
               if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
                  ...
                  /* Get the distance between the light and the loot-bot */
                  ...
                  /* Get the angle wrt to loot-bot rotation */
                  cAngleLightWrtLootbot = cRobotToLight.GetZAngle();
                  cAngleLightWrtLootbot -= cOrientationZ;
                  /*
                   * Find closest sensor index to point at which ray hits lootbot body
                   * Rotate whole body by half a sensor spacing (corresponding to placement of first sensor)
                   * Division says how many sensor spacings there are between first sensor and point at which ray hits lootbot body
                   * Increase magnitude of result of division to ensure correct rounding
                   */
                  Real fIdx = (cAngleLightWrtLootbot - SENSOR_HALF_SPACING) / SENSOR_SPACING;
                  ...
                  for(SInt32 nIndexOffset = -6; nIndexOffset < 7; ++nIndexOffset) {
                     ...
                     CRadians cAngularDistanceFromOptimalLightReceptionPoint = Abs((cAngleLightWrtLootbot - m_tReadings[unIdx].Angle).SignedNormalize());
                     /*
   ...
   void CLootBotLightRotZOnlySensor::Reset() {
   ...
   void CLootBotLightRotZOnlySensor::Enable() {
     m_bEnabled = true;
   }
   ...
   void CLootBotLightRotZOnlySensor::Disable() {
     m_bEnabled = false;
   }
   ...
   REGISTER_SENSOR(CLootBotLightRotZOnlySensor,
                   "lootbot_light", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The loot-bot light sensor (optimized for 2D).",
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <lootbot_light implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
