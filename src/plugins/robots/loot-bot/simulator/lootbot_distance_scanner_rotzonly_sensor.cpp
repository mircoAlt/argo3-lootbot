/**
 * @file "../simulator/lootbot_distance_scanner_rotzonly_sensor.cpp"
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "lootbot_distance_scanner_rotzonly_sensor.h"
...
namespace argos {
   ...
   static const Real LOOTBOT_RADIUS           = 0.085036758f;

   static const Real SHORT_RANGE_MIN_DISTANCE = 0.0f;
   static const Real SHORT_RANGE_RAY_START    = LOOTBOT_RADIUS;
   static const Real SHORT_RANGE_RAY_END      = LOOTBOT_RADIUS + 0.26f;

   static const Real LONG_RANGE_MIN_DISTANCE  = 0.12f;
   static const Real LONG_RANGE_RAY_START     = LOOTBOT_RADIUS;
   static const Real LONG_RANGE_RAY_END       = LOOTBOT_RADIUS + 1.42f;
   ...
   CLootBotDistanceScannerRotZOnlySensor::CLootBotDistanceScannerRotZOnlySensor() :
      m_pcRNG(NULL),
      m_bAddNoise(false),
   ...
   void CLootBotDistanceScannerRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_LootBotDistanceScannerSensor::Init(t_tree);
         /* Show rays? */
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in loot-bot distance scanner rot_z_only sensor.", ex);
      }
   }
   ...
   void CLootBotDistanceScannerRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      m_pcEmbodiedEntity = &(c_entity.GetComponent<CEmbodiedEntity>("body"));
      m_pcControllableEntity = &(c_entity.GetComponent<CControllableEntity>("controller"));
      m_pcDistScanEntity = &(c_entity.GetComponent<CLootBotDistanceScannerEquippedEntity>("distance_scanner"));
   ...
   void CLootBotDistanceScannerRotZOnlySensor::Update() {
      /* Clear the maps */
      m_tReadingsMap.clear();
      m_tShortReadingsMap.clear();
      m_tLongReadingsMap.clear();
      /* Perform calculations only if the sensor is on */
      if(m_pcDistScanEntity->GetMode() != CLootBotDistanceScannerEquippedEntity::MODE_OFF) {
         /* Update the readings wrt to device mode */
         if(m_pcDistScanEntity->GetMode() == CLootBotDistanceScannerEquippedEntity::MODE_POSITION_CONTROL) {
   ...
   void CLootBotDistanceScannerRotZOnlySensor::Reset() {
      /* Clear the maps */
      m_tReadingsMap.clear();
      m_tShortReadingsMap.clear();
      m_tLongReadingsMap.clear();
   ...
   void CLootBotDistanceScannerRotZOnlySensor::UpdateNotRotating() {
      /* Short range [0] */
...
#define ADD_READINGS(RAYS,MAP,MINDIST)          \
   ...
   void CLootBotDistanceScannerRotZOnlySensor::UpdateRotating() {
   ...
   Real CLootBotDistanceScannerRotZOnlySensor::CalculateReadingForRay(const CRay3& c_ray,
                                                                      Real f_min_distance) {
...
#define CALCULATE_LONG_RANGE_RAY(ANGLE,INDEX)               \
   ...
   void CLootBotDistanceScannerRotZOnlySensor::CalculateRaysNotRotating() {
      /* We make the assumption that the loot-bot is rotated only around Z */
      /* Get the loot-bot orientation */
      ...
      /* The short range sensors are oriented along the loot-bot local X */
      m_cDirection = CVector3::X;
      CALCULATE_SHORT_RANGE_RAY(cAbsoluteOrientation, 0);
      /* The short range sensors are oriented along the loot-bot local Y */
   ...
   void CLootBotDistanceScannerRotZOnlySensor::CalculateRaysRotating() {
      /* We make the assumption that the loot-bot is rotated only around Z */
      /* Get the loot-bot orientation */
      CRadians cTmp1, cTmp2, cOrientationZ;
      m_pcEmbodiedEntity->GetOriginAnchor().Orientation.ToEulerAngles(cOrientationZ, cTmp1, cTmp2);
      ...
      /* The long range sensors are oriented along the loot-bot local Y */
      m_cDirection = CVector3::Y;
      CALCULATE_LONG_RANGE_RAY(cAbsoluteOrientation, 0);
   ...
   REGISTER_SENSOR(CLootBotDistanceScannerRotZOnlySensor,
                   "lootbot_distance_scanner", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The loot-bot distance scanner sensor (optimized for 2D).",
                   "This sensor accesses the loot-bot distance scanner sensor. For a complete\n"
                   ...
                   "the loot-bot is always parallel to the XY plane, i.e., it rotates only around\n"
                   "the Z axis. This implementation is faster than a 3D one and should be used\n"
                   "only when the assumption about the loot-bot rotation holds.\n\n"
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <lootbot_distance_scanner implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
