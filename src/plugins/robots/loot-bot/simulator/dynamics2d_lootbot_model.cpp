/**
 * @file "../simulator/dynamics2d_lootbot_model.cpp"
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "dynamics2d_lootbot_model.h"
...
namespace argos {
   ...
   static const Real LOOTBOT_MAX_FORCE                = 15.f;
   static const Real LOOTBOT_MAX_TORQUE               = 150.f;
   ...
   CDynamics2DLootBotModel::CDynamics2DLootBotModel(CDynamics2DEngine& c_engine,
                                                    CLootBotEntity& c_entity) :
      CDynamics2DMultiBodyObjectModel(c_engine, c_entity),
      m_cLootBotEntity(c_entity),
      m_cWheeledEntity(m_cLootBotEntity.GetWheeledEntity()),
      m_cGripperEntity(c_entity.GetGripperEquippedEntity()),
      m_cDiffSteering(c_engine,
                      LOOTBOT_MAX_FORCE,
                      LOOTBOT_MAX_TORQUE,
                      LOOTBOT_INTERWHEEL_DISTANCE,
                      c_entity.GetConfigurationNode()),
      m_pcGripper(NULL),
      m_pcGrippable(NULL),
      m_fMass(1.6f),
      m_fCurrentWheelVelocity(m_cWheeledEntity.GetWheelVelocities()),
      m_unLastTurretMode(m_cLootBotEntity.GetTurretEntity().GetMode()) {
      RegisterAnchorMethod<CDynamics2DLootBotModel>(
         GetEmbodiedEntity().GetOriginAnchor(),
         &CDynamics2DLootBotModel::UpdateOriginAnchor);
      RegisterAnchorMethod<CDynamics2DLootBotModel>(
         GetEmbodiedEntity().GetAnchor("turret"),
         &CDynamics2DLootBotModel::UpdateTurretAnchor);
      RegisterAnchorMethod<CDynamics2DLootBotModel>(
         GetEmbodiedEntity().GetAnchor("perspective_camera"),
         &CDynamics2DLootBotModel::UpdatePerspectiveCameraAnchor);
      ...
      AddBody(m_ptActualBaseBody, cpvzero, 0, LOOTBOT_HEIGHT);
      ...
      m_ptBaseGripperAngularMotion->maxForce = LOOTBOT_MAX_TORQUE; /* limit the dragging torque */
      /* Add the gripper body so that the default methods work as expected */
      AddBody(m_ptActualGripperBody, cpvzero, 0, LOOTBOT_HEIGHT);
   ...
   CDynamics2DLootBotModel::~CDynamics2DLootBotModel() {
      delete m_pcGripper;
   ...
   void CDynamics2DLootBotModel::MoveTo(const CVector3& c_position,
   ...
   void CDynamics2DLootBotModel::Reset() {
   ...
   void CDynamics2DLootBotModel::CalculateBoundingBox() {
      ...
      GetBoundingBox().MaxCorner.SetZ(GetDynamics2DEngine().GetElevation() + LOOTBOT_HEIGHT);
   }

   /****************************************/
   /****************************************/

   void CDynamics2DLootBotModel::UpdateFromEntityStatus() {
      /* Do we want to move? */
      if((m_fCurrentWheelVelocity[LOOTBOT_LEFT_WHEEL] != 0.0f) ||
         (m_fCurrentWheelVelocity[LOOTBOT_RIGHT_WHEEL] != 0.0f)) {
         m_cDiffSteering.SetWheelVelocity(m_fCurrentWheelVelocity[LOOTBOT_LEFT_WHEEL],
                                          m_fCurrentWheelVelocity[LOOTBOT_RIGHT_WHEEL]);
      }
      else {
      ...
      if(m_cLootBotEntity.GetTurretEntity().GetMode() != m_unLastTurretMode) {
         /* Enable or disable the anchor */
         if(m_cLootBotEntity.GetTurretEntity().GetMode() != MODE_OFF) {
         ...
         switch(m_unLastTurretMode) {
            case MODE_OFF:
            case MODE_PASSIVE:
               switch(m_cLootBotEntity.GetTurretEntity().GetMode()) {
                  case MODE_POSITION_CONTROL:
                  case MODE_SPEED_CONTROL:
      ...
      switch(m_unLastTurretMode) {
         ...
         case MODE_POSITION_CONTROL: {
            Real fCurRotErr = NormalizedDifference(
               m_cLootBotEntity.GetTurretEntity().GetDesiredRotation(),
         ...
         case MODE_SPEED_CONTROL:
            m_ptControlGripperBody->w =
               m_cDiffSteering.GetAngularVelocity() +
               m_cLootBotEntity.GetTurretEntity().GetDesiredRotationSpeed();
            break;
         case MODE_OFF:
         case MODE_PASSIVE:
   ...
   void CDynamics2DLootBotModel::TurretPassiveToActive() {
      ...
      m_ptGripperControlAngularMotion->maxForce = LOOTBOT_MAX_TORQUE; /* limit the dragging torque */
   }

   /****************************************/
   /****************************************/

   void CDynamics2DLootBotModel::TurretActiveToPassive() {
   ...
   void CDynamics2DLootBotModel::UpdateOriginAnchor(SAnchor& s_anchor) {
   ...
   void CDynamics2DLootBotModel::UpdateTurretAnchor(SAnchor& s_anchor) {
   ...
   void CDynamics2DLootBotModel::UpdatePerspectiveCameraAnchor(SAnchor& s_anchor) {
   ...
   REGISTER_STANDARD_DYNAMICS2D_OPERATIONS_ON_ENTITY(CLootBotEntity, CDynamics2DLootBotModel);
